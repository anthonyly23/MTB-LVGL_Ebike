...
This application allows users to benchmark LVGL widget demo. Set `DEMO_BENCHMARK` to `1` in the common.mk file to enable the benchmarking for widget demo.
> **Note:** When the `DEMO_BENCHMARK` is set to 1, the build automatically switches to Release mode for accurate measurements.