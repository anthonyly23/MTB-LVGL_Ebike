...
*                    - LVGL core, ebike demo, and drawing functions are placed 
*                      in ITCM (fast instruction memory) via linker script
*                    - Style cache enabled for faster property lookups
*                    - Optimized refresh period (20ms)
*
...
static void cm55_gfx_task(void *arg)
{
    ...
    if (CY_GFX_SUCCESS == gfx_status)
    {
        ...
        if (VG_LITE_SUCCESS == vglite_status)
        {
            ...
            /* Show performance monitor (CPU and FPS) */
