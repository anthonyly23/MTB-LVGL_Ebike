                                             ...
                                             uint8_t *color_p)
{
    ...
    /* Drain any stale vsync notifications that arrived during LVGL rendering.
     * This prevents the race condition where we consume a stale notification
     * and mark the buffer ready before DC actually swaps to the new framebuffer. */
